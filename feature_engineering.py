...
if __name__ == "__main__":
    ...
    if not raw.empty:
        ...
        categories = {
            "Trend":     [f for f in feat_cols if any(x in f for x in ["ema","macd","adx","vwap","ichi","psar","super"])],
            "Momentum":  [f for f in feat_cols if any(x in f for x in ["rsi","stoch","mfi","williams","roc","cmo","dpo","cci"])],
            "Volatility":[f for f in feat_cols if any(x in f for x in ["bb","kc","atr","natr","don","hvol","sqz","vix"])],
            "Volume":    [f for f in feat_cols if any(x in f for x in ["obv","cmf","ad_","rvol","vol_"])],
            "F&O":       [f for f in feat_cols if any(x in f for x in ["oi","pcr","max_pain","futures","dte","iv","gex"])],
            "Macro":     [f for f in feat_cols if any(x in f for x in ["fii","dii","spx","ndx","usdinr","gold","crude"])],
            "Derived":   [f for f in feat_cols if any(x in f for x in ["ret_","regime","dow","dom","month","drawdown","midcap"])],
        }
